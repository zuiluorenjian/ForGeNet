...
class RandomMask(object):
    """Randomly mask non-overlapping image patches."""
...
def translate_duplicate(img, cropSize=224):
    """Tile a small image until both dimensions satisfy the crop size."""
...
class RealFakeDataset(Dataset):
    """Load labeled real and fake images with train or validation transforms."""
    def __init__(self, opt, is_pretraining=False):
        ...
        duplicate_func = transforms.Lambda(lambda img: translate_duplicate(img, opt.cropSize))
        ...
        if '2b' not in opt.arch:
            use_forgelens_style = getattr(opt, 'use_forgelens_style', False)

            if use_forgelens_style:
                print("Using ForgeLens-style preprocessing without extra augmentation")
                ...
                print("Using UniversalFakeDetect-style preprocessing with augmentation")
                self.transform = transforms.Compose([
...
def data_augment(img, opt):
    """Apply optional blur and JPEG perturbations."""
    img = np.array(img)
    if img.ndim == 2:
